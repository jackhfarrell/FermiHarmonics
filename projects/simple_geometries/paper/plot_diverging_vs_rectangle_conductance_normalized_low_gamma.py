...
OUTFILE = PAPER_DIR / "all_geometries_conductance_normalized_low_gamma.png"
...
def main():
    ...
    g_rect_norm = g_rect / g_rect[0]  # normalize by each geometry's low-gamma conductance
    g_div_norm = g_div / g_div[0]
    g_dog_norm = g_dog / g_dog[0]
    ...
    ax.set_ylabel(r"$G(\gamma_{mc}) / G(\gamma_{mc,\min})$")
    ax.set_title("Conductance Normalized By Low-$\\gamma_{mc}$ Value (All Geometries)")
