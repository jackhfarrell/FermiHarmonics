# run a large parameter sweep for the simple_geometries chamber problem, sweeping over gamma_mr and gamma_mc using
...
# sweep configuration
name = "simple_geometries_chamber"
...
source_bias = 0.5
drain_left_bias = -0.5
drain_center_bias = -0.5
drain_right_bias = -0.5
...
boundary_conditions = Dict(
    ...
    :contact_source => OhmicContactBC(source_bias),
    :drain_left => OhmicContactBC(drain_left_bias),
    :drain_center => OhmicContactBC(drain_center_bias),
    :drain_right => OhmicContactBC(drain_right_bias),
)
...
solve_params = SolveParams(;
    ...
    polydeg = 3,
    tspan_end = 300.0, # end time for simulation (if not converged earlier)
    residual_tol = 1e-5,
    cfl = 0.5,
    log_every = 500,
)
...
sweep_metadata = Dict(
    ...
    "drain_left_bias" => drain_left_bias,
    "drain_center_bias" => drain_center_bias,
    "drain_right_bias" => drain_right_bias,
)
...
if !haskey(ENV, "SLURM_ARRAY_TASK_ID")
    ...
    @info "Chamber sweep submission" name mesh=basename(mesh_path) bias p_scatter total_cases
