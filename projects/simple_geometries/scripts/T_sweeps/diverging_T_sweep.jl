# run a T-only parameter sweep for the simple_geometries diverging problem using
...
# sweep configuration
name = get(ENV, "FH_DIV_T_NAME", "simple_geometries_diverging_T")
project_root = normpath(joinpath(@__DIR__, "..", ".."))
main_project = normpath(joinpath(project_root, "..", ".."))
mesh_path = get(ENV, "FH_DIV_T_MESH_PATH", joinpath(project_root, "meshes", "diverging", "diverging.inp"))
results_root = get(ENV, "FH_DIV_T_RESULTS_ROOT", joinpath(project_root, "results"))
mkpath(results_root)

# local mode: run all T points in-process without submitting slurm (default false)
run_local = lowercase(get(ENV, "FH_DIV_T_RUN_LOCAL", "false")) in ("1", "true", "yes", "on")

# physical and sweep parameters
bias = parse(Float64, get(ENV, "FH_DIV_T_BIAS", "1.0"))
p_scatter = parse(Float64, get(ENV, "FH_DIV_T_P_SCATTER", "1.0"))
a_mc = parse(Float64, get(ENV, "FH_DIV_T_A_MC", "1.0"))
a_mr = parse(Float64, get(ENV, "FH_DIV_T_A_MR", "1.0"))

Tmin = parse(Float64, get(ENV, "FH_DIV_T_TMIN", "1e-3"))
Tmax = parse(Float64, get(ENV, "FH_DIV_T_TMAX", "1e-1"))
nT = parse(Int, get(ENV, "FH_DIV_T_NT", "40"))
chunk_size = parse(Int, get(ENV, "FH_DIV_T_CHUNK_SIZE", "8"))

warm_start = lowercase(get(ENV, "FH_DIV_T_WARM_START", "true")) in ("1", "true", "yes", "on")

# solver parameters
solve_params = SolveParams(;
    min_harmonic = parse(Int, get(ENV, "FH_DIV_T_MIN_HARMONIC", "4")),
    max_harmonic_auto = parse(Int, get(ENV, "FH_DIV_T_MAX_HARMONIC_AUTO", "100")),
    polydeg = parse(Int, get(ENV, "FH_DIV_T_POLYDEG", "3")),
    tspan_end = parse(Float64, get(ENV, "FH_DIV_T_TSPAN_END", "100.0")),
    residual_tol = parse(Float64, get(ENV, "FH_DIV_T_RESIDUAL_TOL", "1e-5")),
    cfl = parse(Float64, get(ENV, "FH_DIV_T_CFL", "0.5")),
    log_every = parse(Int, get(ENV, "FH_DIV_T_LOG_EVERY", "500")),
)

max_harmonic_env = lowercase(strip(get(ENV, "FH_DIV_T_MAX_HARMONIC", "auto")))
max_harmonic = max_harmonic_env == "auto" ? :auto : parse(Int, max_harmonic_env)

n_threads = parse(Int, get(ENV, "FH_DIV_T_N_THREADS", "1"))

# slurm parameters
sbatch = Dict(
    :job_name => "$(name)_sweep",
    :time => get(ENV, "FH_DIV_T_SBATCH_TIME", "24:00:00"),
    :cpus_per_task => parse(Int, get(ENV, "FH_DIV_T_SBATCH_CPUS_PER_TASK", "1")),
    :mem_per_cpu => get(ENV, "FH_DIV_T_SBATCH_MEM_PER_CPU", "4G"),
    :account => get(ENV, "FH_DIV_T_SBATCH_ACCOUNT", "ucb485_asc2"),
    :partition => get(ENV, "FH_DIV_T_SBATCH_PARTITION", "amilan"),
    :qos => get(ENV, "FH_DIV_T_SBATCH_QOS", "normal"),
    :export => "ALL",
)
...
if !haskey(ENV, "SLURM_ARRAY_TASK_ID") && !run_local
    @info "Diverging T sweep submission" name mesh=basename(mesh_path) Tmin Tmax nT chunk_size n_jobs a_mc a_mr

    timestamp = Dates.format(now(), "yyyy-mm-dd_HHMMSS")
    tag = param_hash(; a_mc, a_mr, Tmin, Tmax, nT, chunk_size, bias, p_scatter)
    sweep_id = "$(name)_sweep_diverging_$(tag)_$(timestamp)"
...
if haskey(ENV, "SLURM_ARRAY_TASK_ID")
    ...
    write_sweep_metadata!(sweep_dir, solve_params, sweep_metadata, slurm_metadata, gamma_mr_vals, gamma_mc_vals)
    archive_mesh!(mesh_path, sweep_dir)
    @info "Running local diverging sweep" output_dir=sweep_dir cases=length(assigned_indices)
end
