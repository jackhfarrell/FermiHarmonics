# run a gamma_ee-only parameter sweep for the simple_geometries junction problem at fixed gamma_mr using
...
# sweep configuration
name = "simple_geometries_junction_gamma_ee_sweep"
...
gamma_ee_vals = log_base .^ range(log(log_base, 1e3), log(log_base, 1e-2), length=n_gamma_ee)
...
if !haskey(ENV, "SLURM_ARRAY_TASK_ID")
    ...
    @info "Junction gamma_ee sweep submission" name mesh=basename(mesh_path) bias p_scatter gamma_mr_fixed total_cases
