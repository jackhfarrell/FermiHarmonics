# run a gamma_mc-only parameter sweep for the simple_geometries junction problem at fixed gamma_mr using
...
sbatch = Dict(
    ...
    :nodes => 1,
    :ntasks => 1,
    :cpus_per_task => n_threads,
    :tres_per_task => "cpu=$(n_threads)",
    :mem_per_cpu => "8G",
    :account => "ucb485_asc2",
    :partition => "amilan",
    :qos => "normal",
...
gamma_mc_vals = log_base .^ range(log(log_base, 1e3), log(log_base, 1e-2), length=n_gamma_mc)
...
slurm_metadata = Dict(
    ...
    "tres_per_task" => sbatch[:tres_per_task],
...
if !haskey(ENV, "SLURM_ARRAY_TASK_ID")
    ...
    @info "Junction gamma_mc sweep submission" name mesh=basename(mesh_path) bias p_scatter gamma_mr_fixed total_cases
