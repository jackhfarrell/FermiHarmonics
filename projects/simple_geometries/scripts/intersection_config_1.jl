# run a large parameter sweep for the simple_geometries intersection problem, sweeping over gamma_mr and gamma_mc
# using SLURM array jobs. this script handles both the initial submission (writing shared metadata and submitting the
# array job) and the worker logic. the parameter grid is defined in the script, and each worker picks which cases to
# run based on its SLURM_ARRAY_TASK_ID. results are saved in a shared directory with one file per case.
...
# sweep configuration
name = "simple_geometries_intersection_config_1"
...
bias = 1.0 # nominal top-to-bottom drive for metadata/filenames
p_scatter = 1.0 # scattering prob. at walls (1.0 = fully diffuse, 0.0 = fully specular)

bottom_contact_bias = 0.5
top_contact_bias = -0.5
right_contact_bias = -0.5 # right arm is labeled "contact_side" in this mesh
...
boundary_conditions = Dict(
    ...
    :contact_bottom => OhmicContactBC(bottom_contact_bias),
    :contact_top => OhmicContactBC(top_contact_bias),
    :contact_side => OhmicContactBC(right_contact_bias),
)
...
sweep_metadata = Dict(
    "bias" => bias,
    "p_scatter" => p_scatter,
    "bottom_contact_bias" => bottom_contact_bias,
    "top_contact_bias" => top_contact_bias,
    "right_contact_bias" => right_contact_bias,
)
...
if !haskey(ENV, "SLURM_ARRAY_TASK_ID")
    ...
    @info "Intersection sweep submission" name mesh=basename(mesh_path) bias p_scatter total_cases
