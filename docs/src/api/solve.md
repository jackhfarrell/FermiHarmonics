...
- `MeshBuildConfig(mesh_scale=3.0)` uses temporary unique output paths under `SLURM_TMPDIR` / `TMPDIR` / `tempdir()`.
- The default Gmsh `algorithm=8` targets quasi-structured quadrilateral meshes with `recombine_all=true`.
