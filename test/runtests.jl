...
@testset "FermiHarmonics smoke tests" begin
    ...
    @test estimate_max_harmonic(0.0, 0.0; min_harmonic=4, max_harmonic=150) == 150
    @test estimate_max_harmonic(0.0, 1000.0; min_harmonic=4, max_harmonic=150) == 4

    eq = FermiHarmonics2D(9; gamma_mr=0.1, gamma_ee=1.0, max_harmonic=4)
    eq_mag = FermiHarmonics2D(9; gamma_mr=0.1, gamma_ee=1.0, omega_c=0.25, max_harmonic=4)
    @test typeof(eq) <: Trixi.AbstractEquations{2, 9}
    @test eq.omega_c == 0.0
    @test eq.gamma_ee ≈ 1.0 atol=1e-12 rtol=1e-12
    @test eq.gamma_3 == 0.0
    @test eq_mag.omega_c ≈ 0.25 atol=1e-12 rtol=1e-12
    @test occursin("ω_c=0.25", sprint(show, eq_mag))
    @test occursin("γ_ee=1.0", sprint(show, eq))
    ...
    @test source_zero[2] ≈ -0.1 * u[2] atol=1e-12 rtol=1e-12
    @test source_zero[3] ≈ -0.1 * u[3] atol=1e-12 rtol=1e-12
    @test source_zero[4] ≈ -(0.1 + 1.0) * u[4] atol=1e-12 rtol=1e-12
    @test source_zero[5] ≈ -(0.1 + 1.0) * u[5] atol=1e-12 rtol=1e-12
    @test source_zero[6] ≈ -(0.1 + 1.0) * u[6] atol=1e-12 rtol=1e-12
    @test source_zero[7] ≈ -(0.1 + 1.0) * u[7] atol=1e-12 rtol=1e-12
    ...
    eq_tomo = FermiHarmonics2D(9; gamma_mr=0.1, gamma_ee=1.0, gamma_3=0.2, max_harmonic=4)
    source_tomo = FermiHarmonics.physical_sources(u, nothing, 0.0, eq_tomo)
    expected_odd_rate = 0.1 + 1.0 + min(0.2 * 3^4, 1.0)
    @test source_tomo[6] ≈ -expected_odd_rate * u[6] atol=1e-12 rtol=1e-12
    @test source_tomo[7] ≈ -expected_odd_rate * u[7] atol=1e-12 rtol=1e-12

    pure_mode = zeros(9)
    pure_mode[4] = 1.0
    pure_mode_source = FermiHarmonics.physical_sources(pure_mode, nothing, 0.0, FermiHarmonics2D(
        9; gamma_mr=0.0, gamma_ee=0.0, omega_c=0.5, max_harmonic=4,
    ))
    ...
    sol_tomo, semi_tomo = solve(
        ...
        max_harmonic=2,
        gamma_3=0.2,
        name="test_tomographic_rates",
    )
    @test semi_tomo.equations.gamma_ee ≈ 0.5 atol=1e-12 rtol=1e-12
    @test semi_tomo.equations.gamma_3 ≈ 0.2 atol=1e-12 rtol=1e-12
