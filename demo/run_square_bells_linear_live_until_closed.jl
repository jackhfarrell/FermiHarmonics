...
function main()
    ...
    run_name = "square_bells_linear_live_gamma_mc01_M60_" * run_stamp
    ...
    live_visualization = LiveVisualizationConfig(;
        ...
        field=:jx,
        nvisnodes=200,
        accepted_step_interval=10,
        min_update_seconds=0.2,
        show_window=true,
    )

    @info "Running square-bells linear live demo" mesh_path gamma_mr gamma_mc max_harmonic bias run_name
    @info "Close the live window to stop the solve and save the current state" output_dir

    sol, semi = FermiFlows.solve(
        FermiFlows.TrixiProblem(; mesh_path=mesh_path, boundary_conditions=boundary_conditions),
        model,
        config;
    ...
    FermiFlows.save_solution_custom(sol, semi, restart_path)
    FermiFlows.save_mesh_native_analysis(sol, semi, analysis_path; refine=4)
    ...
    status = FermiFlows.solve_status(sol, semi, config; stop_reason_override=stop_reason_override)
