...
"""
    estimate_max_harmonic(gamma_mr, gamma_ee; min_harmonic=4, max_harmonic=150,
                          decay_power=1.6)
...
function solve(mesh_path::AbstractString, boundary_conditions::Dict{Symbol, Any},
    ...
    @info "Starting solve" name=name max_harmonic=max_harmonic_resolved harmonic_mode=harmonic_mode gamma_mr=gamma_mr gamma_ee=gamma_ee gamma_3=gamma_3 omega_c=omega_c polydeg=params.polydeg cfl=params.cfl residual_tol=params.residual_tol residual_mode=params.residual_mode residual_nvars=residual_nvars residual_scale_floor=params.residual_scale_floor boundaries=boundary_types
    flush(stdout)
    flush(stderr)
